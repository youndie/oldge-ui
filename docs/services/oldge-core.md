...
| `oldge-core/src/commonMain/kotlin/io/github/youndie/oldge/feedback/` | `OldgeReadout`, `OldgeMeter`, `OldgeProgressBar` (B-29), `OldgeSpinner`, `OldgeSkeleton` (B-30), `OldgeBadge`, `OldgeAvatar` (B-31), `OldgeBanner`, `OldgeSnackbar`, `OldgeEmptyState` (B-32), `OldgePullRefresh` (B-33), with the LCD tag and text in `Lcd.kt` and the frozen loop phase in `Loop.kt`; `LcdBehaviourTest`, `LoopBehaviourTest`, `BadgeAvatarBehaviourTest`, `NoticeBehaviourTest` and `PullRefreshBehaviourTest` hold their READMEs' behaviour rules |
