...
# B-17 — TextField and SearchBar

TextField: label above, a sunken white well, help and error lines (error = colour + icon + word), password reveal, multiline, minimum rather than fixed height. SearchBar: the sunken capsule. Built on `BasicTextField` (research D2).

- **References**: `TextField_{Toxic,Media,Crystal}`, `SearchBar_{Toxic,Media,Crystal}` in `oldge-core/src/desktopTest/snapshots/design/`.
